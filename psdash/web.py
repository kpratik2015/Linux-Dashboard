...
@webapp.route('/apps')
def view_apps():
	os.system("pip list --format json > apps_list.json")
	# f = open('apps_list.json', 'r')
	# data = json.load(f.read())
	with open("apps_list.json") as apps_list:    
		data = json.load(apps_list)
	# pprint(data([0]))
	apps = {}
	# app_names = []
	# app_versions = []
	...
	#	app_names.append(app_data['version'])
	#	print( + " " + app_data['version'])
	for app_data in data:
		apps[app_data['name']] = app_data['version']
	return render_template(
		'apps.html',
		page='apps',
		apps=apps,
