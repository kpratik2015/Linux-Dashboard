...
from psdash.node import LocalNode, RemoteNode, LocalService
...
class PsDashRunner(object):
	...
	def _create_app(self, config=None):
		...
		app.config.update(
			...
			PSDASH_AUTH_USERNAME="redhat",
			PSDASH_AUTH_PASSWORD="redhat"
		)
	...
	def _net_io_counters_worker(self, sleep_interval):
		static_url_path = os.path.dirname(os.path.realpath(__file__))+'/static/'
		# static_folder = self.app.instance_path + static_url_path
		# print(static_folder)
		...
		while True:
			...
			user_procs = LocalService(self.get_local_node()).get_user_process_list()
			# print(user_procs)
			...
			try:
				...
				with open(static_url_path+'/data.json', 'w') as outfile:
	...
	def run(self):
		logger.info('Starting PatDash v%s' % __version__)
